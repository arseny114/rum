 ...
 *
 * rum_vacuum.out - test output for PostgreSQL (<18)
 * rum_vacuum_1.out - for PostgreSQL (>=18)
 */
...
EXPLAIN (costs off)
SELECT count(*) FROM (SELECT * FROM test_rum_vacuum_backward WHERE body @@ 'ann'::tsquery ORDER BY id <=| 2) AS sub;
SELECT count(*) FROM (SELECT * FROM test_rum_vacuum_backward WHERE body @@ 'ann'::tsquery ORDER BY id <=| 2) AS sub;

EXPLAIN (costs off)
SELECT count(*) FROM (SELECT * FROM test_rum_vacuum_backward WHERE body @@ 'john'::tsquery ORDER BY id <=| 2) AS sub;
SELECT count(*) FROM (SELECT * FROM test_rum_vacuum_backward WHERE body @@ 'john'::tsquery ORDER BY id <=| 2) AS sub;
